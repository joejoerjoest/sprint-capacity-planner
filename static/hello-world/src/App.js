...
export default function App() {
  ...
  const [confSpaceId, setConfSpaceId] = useState('');
  const [confPageId, setConfPageId] = useState('');
  const [confPageUrl, setConfPageUrl] = useState('');
  ...
  // ── Confluence publishing ──
  async function loadConfSpaces() {
    setConfBusy('spaces'); setConfError(''); setConfMsg('');
    try {
      const res = await invoke('confluenceSpaces');
      const spaces = res?.spaces ?? [];
      setConfSpaces(spaces);
      if (spaces.length === 0) setConfError('No Confluence spaces found (is Confluence set up on this site?).');
    } catch (e) {
      console.error(e); setConfError(e.message || 'Failed to load Confluence spaces.');
    } finally {
      setConfBusy('');
    }
  }
  ...
  function buildConfluenceBody() {
    ...
    const showCommitted = committed && committed.hasSP;
    let h = '';
    h += `<p><strong>${esc(sprintName)}</strong> &middot; ${esc(sprintStart)} &rarr; ${esc(sprintEnd)}</p>`;
    h += `<p>Working days: ${sprintDays} &middot; Members: ${members.length} &middot; Available: ${Math.round(totalHours * 10) / 10}h / ${Math.round(totalSP * 10) / 10} SP &middot; Avg capacity: ${avgPct}%${bufferPct > 0 ? ` &middot; Buffer: ${bufferPct}%` : ''}</p>`;
    h += '<table><tbody><tr><th>Name</th><th>Role</th><th>Working days</th><th>Leave</th><th>Hours</th><th>SP</th><th>Capacity</th>';
    if (showCommitted) h += '<th>Committed SP</th>';
    h += '</tr>';
    for (const r of [...results].sort((a, b) => a.pct - b.pct)) {
      h += `<tr><td>${esc(r.name)}</td><td>${esc(r.role || '-')}</td><td>${r.availDays}/${r.totalDays}</td><td>${r.leaveDays}d</td><td>${r.availHours}h</td><td>${r.availSP}</td><td>${r.pct}%</td>`;
      if (showCommitted) { const cm = committedForMember(r); const c = cm ? cm.sp : 0; h += `<td>${c}${c > r.availSP ? ' (over)' : ''}</td>`; }
      h += '</tr>';
    }
    h += '</tbody></table>';
    if (showCommitted) h += `<p>Team committed in Jira: <strong>${committed.totalSP} SP</strong> across ${committed.issueCount} issues vs available <strong>${Math.round(totalSP * 10) / 10} SP</strong>.</p>`;
    h += `<p><em>Generated by Sprint Capacity Planner on ${new Date().toLocaleString()}</em></p>`;
    return h;
  }

  async function publishToConfluence() {
    if (!confSpaceId) { setConfError('Pick a Confluence space first.'); setConfMsg(''); return; }
    if (members.length === 0) { setConfError('Add team members first.'); setConfMsg(''); return; }
    setConfBusy('publish'); setConfError(''); setConfMsg('');
    try {
      const title = `Sprint Capacity — ${sprintName}`;
      const body = buildConfluenceBody();
      const res = await invoke('publishConfluence', { spaceId: confSpaceId, title, body, pageId: confPageId || null });
      if (res?.error) { setConfError(res.error); return; }
      if (res?.pageId) setConfPageId(res.pageId);
      setConfPageUrl(res?.url || '');
      setConfMsg(res?.updated ? 'Updated the Confluence page.' : 'Published a new Confluence page.');
    } catch (e) {
      console.error(e); setConfError(e.message || 'Publish failed.');
    } finally {
      setConfBusy('');
    }
  }
            ...
            : sprintStart >= sprintEnd
              ...
              : (
                <div>
                  ...
                  <div style={{ ...styles.card, marginTop: '16px', borderLeft: '4px solid #4299e1' }}>
                    <p style={{ ...styles.sectionHeader, border: 'none', margin: 0, padding: 0 }}>📤 Publish to Confluence</p>
                    <p style={{ fontSize: '12px', color: '#718096', margin: '6px 0 12px' }}>
                      Create (or update) a Confluence page with this report — summary, per-member table, and committed-vs-available.
                    </p>
                    <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
                      <button style={styles.btnSecondary} onClick={loadConfSpaces} disabled={confBusy === 'spaces'}>
                        {confBusy === 'spaces' ? 'Loading…' : '⟳ Load spaces'}
                      </button>
                      {confSpaces.length > 0 && (
                        <select style={styles.select} value={confSpaceId} onChange={(e) => setConfSpaceId(e.target.value)}>
                          <option value="">Select a space…</option>
                          {confSpaces.map((s) => <option key={s.id} value={s.id}>{s.name} ({s.key})</option>)}
                        </select>
                      )}
                      <button style={styles.btnPrimary} onClick={publishToConfluence} disabled={confBusy === 'publish' || !confSpaceId}>
                        {confBusy === 'publish' ? 'Publishing…' : (confPageId ? '⤴ Update page' : '⤴ Publish page')}
                      </button>
                    </div>
                    {confMsg && (
                      <p style={{ fontSize: '12px', color: '#9ae6b4', margin: '10px 0 0' }}>
                        {confMsg}{confPageUrl ? <> — <a href={confPageUrl} target="_blank" rel="noreferrer" style={{ color: '#90cdf4' }}>open page</a></> : ''}
                      </p>
                    )}
                    {confError && <p style={{ fontSize: '12px', color: '#fc8181', margin: '10px 0 0' }}>{confError}</p>}
                  </div>
                </div>
              )
          }
